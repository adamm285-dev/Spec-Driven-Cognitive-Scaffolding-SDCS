...
def test_subagent_fork_and_rollup(tmp_path):
    ...
    worker_file = fork_subagent_state(tmp_path, "worker-api", subtask_objective="Implement auth endpoints")
    ...
    success, msg = rollup_subagent_state(tmp_path, "worker-api", delete_after_rollup=True)
...
def test_subagent_cli_commands(tmp_path):
    ...
    # Fork CLI
    cmd_fork = [sys.executable, "-m", "sdcs.cli", "state", "fork", "worker-ui", "--repo-root", str(tmp_path)]
    ...
    # Rollup CLI
    cmd_rollup = [sys.executable, "-m", "sdcs.cli", "state", "rollup", "worker-ui", "--repo-root", str(tmp_path)]
